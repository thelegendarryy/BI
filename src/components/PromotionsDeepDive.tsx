...
export const PromotionsDeepDive: React.FC = () => {
  ...
  const chartTheme = {
    promoBar: isDark ? '#818cf8' : '#4f46e5',      // soft indigo vs solid indigo
    volumeBar: isDark ? '#60a5fa' : '#2563eb',     // soft blue vs solid blue
    marginLine: isDark ? '#34d399' : '#10b981',    // soft emerald vs solid emerald
    text: isDark ? '#94a3b8' : '#64748b',          // slate-400 vs slate-505
  ...
  const SensitivityTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      // Find payload index corresponding to volume and margin
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 border-b border-border/40 pb-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Comparison of overall sales volume and revenue by promotion type dimensions.
            </p>
          </div>
        </div>
      ...
      {/* Discount Rate Sensitivity Analysis (Quantity vs Margin %) */}
      <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center gap-3 border-b border-border/40 pb-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Dual-axis mapping showing how higher discount percentages affect aggregate transaction volume vs overall profit margins.
            </p>
          </div>
        </div>
        ...
        <div className="p-4 bg-primary/[0.02] border border-primary/10 rounded-2xl space-y-2 mt-4">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            The data demonstrates standard retail price elasticity. Zero discount sales generate the highest average gross profit margin (~53%), but limit transaction velocity. Transitioning to higher discount brackets (e.g. 10% to 15%) drives significantly larger volume sales (especially from Wholesale and Corporate customer segments), yet the profit margins decay steadily. Black Friday (20% discount) results in the highest volume spikes, but represents the lowest profit margin point (~34%), illustrating the inflection trade-off between volume scale and net returns.
          </p>
        </div>
      </div>
    </div>
